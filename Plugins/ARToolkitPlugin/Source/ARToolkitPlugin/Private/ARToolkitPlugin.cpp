...
{
	if (!processingTexture && newFrame==false){
		int len = LocalJNIEnv->GetArrayLength(data);
		//FPlatformMisc::LowLevelOutputDebugStringf(TEXT("Buffer length: '%d'\n"), len);
		//Copy webcam data to the buffer
		FMemory::Free(Buffer);
		Buffer = FMemory::Malloc(len);
		LocalJNIEnv->GetByteArrayRegion(data, 0, len, reinterpret_cast<jbyte*>(Buffer));
		newFrame = true;
